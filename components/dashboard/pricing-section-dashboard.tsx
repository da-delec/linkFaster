...
const plans = [
  ...
  },
  {
    ...
    features: [
      'Up to 8 freelance platform links',
...
export default function SimplePricingDashboard() {
  ...
  const { 
   ...
   console.log(session?.user)
    const handleCheckout = async (planId: string,period:string) => {
      ...
      try {
        const response = await fetch('/api/create-checkout', {
          ...
          body: JSON.stringify({
            customerId: (session?.user as any)?.stripeCustomerId,
            period: frequency
              ...
              >
                    ...
                    <div className="pt-2">
                      {typeof plan.price[
                        ...
                        <div className="flex items-baseline">
                          ...
                          <h1 className=' ml-3'>{plan.annualPrice[frequency as keyof typeof plan.annualPrice]} $US Annual</h1>
                ...
                <CardFooter>
                  {plan.id === 'free' ? (
                    <Button
                      variant="outline"
                      className="w-full font-medium transition-all duration-300 hover:border-primary/30 hover:bg-primary/5 hover:text-primary"
                      disabled
                    >
                      {plan.cta}
                    ...
                    <Button
                      onClick={() => handleCheckout(plan.id,frequency)}
